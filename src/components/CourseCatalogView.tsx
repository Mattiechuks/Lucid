      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {courses.map((course) => {
          ...
          return (
            <div
              key={course.code}
              className="bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-all duration-200 flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="font-mono text-xs font-bold text-neutral-900 px-2.5 py-1 bg-neutral-100 rounded-lg border border-neutral-200">
                    {course.code}
                  </span>
                ...
                <h3 className="font-serif-display text-lg font-bold text-neutral-900 mb-2">
                  {course.title}
                </h3>

                <div className="grid grid-cols-3 gap-2 py-3 px-3.5 rounded-xl bg-neutral-50/80 border border-neutral-200/60 my-4 text-center">
                  <div>
                    <span className="block text-base font-bold text-neutral-900">{courseNotes.length}</span>
                    <span className="text-[10px] text-neutral-500 uppercase tracking-wider font-semibold">Notes</span>
                  </div>
                  <div>
                    <span className="block text-base font-bold text-neutral-900">{totalCards}</span>
                    <span className="text-[10px] text-neutral-500 uppercase tracking-wider font-semibold">Cards</span>
                  </div>
                  <div>
                    <span className="block text-base font-bold text-neutral-900">{coursePapers.length}</span>
                    <span className="text-[10px] text-neutral-500 uppercase tracking-wider font-semibold">Papers</span>
                  </div>
                </div>
              </div>
